...
// solution
// เข้าถึงข้อมูลlocation ดึงข้อมูลนั้นออกมา
// เข้าถึงข้อมูล total ดึงข้อมูลนั้นออกมา
// กรอกเอาlocationที่ซ้ำกันออก เเล้วรวมtotalเเต่ละlocation เข้าด้วยกัน

// ตัวแปรเก้บจังหวัดที่เอาที่ซ้ำกันออกเเล้ว
...
// ตัวแปรที่เก็บยอดรวมของเเต่ละจังหวัดที่เอาจังหวัดซ้ำกันออกเเล้ว
function findTotal(city) {
  return bills
    .filter((item) => {
...
let sumTotal = location.map(findTotal);
console.log(sumTotal);

const totalPaidByLocation = {};
for (let i = 0; i < location.length; i++) {
  totalPaidByLocation[location[i]] = sumTotal[i];
}
